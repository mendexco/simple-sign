...
import type { DocumentListedWithReceiver } from '@entities/document'
...
const COLUMNS = [
  { name: 'DESTINATION', uid: COLUMNS_UID.DESTINATION },
...
export default function FilesSentPage() {
  const { listSentFiles } = useDocuments()
  const { data, isFetching } = useQuery({
    initialData: [],
    queryFn: listSentFiles,
    queryKey: ['documents', 'listSentFiles']
  })
  ...
  const renderCell = useCallback((document: DocumentListedWithReceiver, columnKey: COLUMNS_UID) => {
    switch (columnKey) {
      case COLUMNS_UID.DESTINATION:
        return (
          <User
            avatarProps={{ name: document.receiver.name?.charAt(0), radius: 'lg' }}
